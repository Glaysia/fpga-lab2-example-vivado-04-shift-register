...
```powershell
git clone --branch v2.0.0 https://github.com/Glaysia/fpga-lab-template.git lab2_04_shift_register
cd lab2_04_shift_register
git switch -c main
code LAB1.code-workspace
```

기능 코어: `shift_register4` / Vivado 설계 top: `lab2_shift_register` / 시뮬레이션 top: `tb_shift_register4`.

- `src/shift_register4.v`: 직접 학습하는 회로.
- `src/input_frontend.v`: 리셋 해제 동기화, 두 단계 입력 동기화, 20 ms 버튼 디바운스와 한 클록 펄스.
- `src/lab2_shift_register.v`: 입력·출력을 코어와 보드 핀 이름에 연결.
- `sim/tb_shift_register4.sv`: 기능 코어의 자기검사. 보드 핀 검증과 구분한다.
- `constraints/lab2_shift_register.xdc`: 19개 포트, S75 핀과 LVCMOS33, 1 kHz 주 클록 제약.
